...
CREATE TABLE `person_economic_problems` (
  ...
  `high_cost_living` tinyint(1) DEFAULT '0',
...
CREATE TABLE `persons` (
  ...
  `contact_number` varchar(20) DEFAULT NULL,
  `user_id` int DEFAULT NULL,
  `census_id` varchar(50) DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `census_id` (`census_id`),
  KEY `idx_persons_user_id` (`user_id`),
  KEY `idx_persons_census_id` (`census_id`),
...
CREATE TABLE `schedule_proposals` (
  ...
  CONSTRAINT `schedule_proposals_ibfk_1` FOREIGN KEY (`blotter_case_id`) REFERENCES `blotter_cases` (`id`) ON DELETE CASCADE,
  CONSTRAINT `schedule_proposals_ibfk_2` FOREIGN KEY (`proposed_by_user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
