...
try {
    ...
    // Retrieve document type code for attribute mapping
    $stmt = $pdo->prepare("SELECT code FROM document_types WHERE id = ?");
    $stmt->execute([$docTypeId]);
    $docCode = $stmt->fetchColumn();

    /* ─────── extra attributes ─────── */
    $attrs = [];
    switch ($docCode) {
        case 'barangay_clearance':
            $attrs['clearance_purpose'] = $_POST['purposeClearance'] ?? null;
            break;
        case 'proof_of_residency':
            $attrs['residency_duration'] = $_POST['residencyDuration'] ?? null;
            $attrs['residency_purpose']  = $_POST['residencyPurpose'] ?? null;
            break;
        case 'barangay_indigency':
            $attrs['indigency_reason']   = $_POST['indigencyReason'] ?? null;
            break;
        case 'cedula':
            $attrs['cedula_occupation']  = $_POST['cedulaOccupation'] ?? null;
            $attrs['cedula_income']      = $_POST['cedulaIncome'] ?? null;
            $attrs['cedula_birthplace']  = $_POST['cedulaBirthplace'] ?? null;
            break;
        case 'business_permit_clearance':
            $attrs['business_name']      = $_POST['businessName'] ?? null;
            $attrs['business_type']      = $_POST['businessType'] ?? null;
            $attrs['business_address']   = $_POST['businessAddress'] ?? null;
            $attrs['business_purpose']   = $_POST['businessPurpose'] ?? null;
            break;
        case 'community_tax_certificate':
            $attrs['ctc_occupation']     = $_POST['ctcOccupation'] ?? null;
            $attrs['ctc_income']         = $_POST['ctcIncome'] ?? null;
            $attrs['ctc_property_value'] = $_POST['ctcPropertyValue'] ?? null;
            $attrs['ctc_birthplace']     = $_POST['ctcBirthplace'] ?? null;
            break;
    }
    ...
    foreach ($attrs as $k => $v) {
        if ($v !== null && trim($v) !== '') {
            // Get attribute_type_id using the attribute code and document type
